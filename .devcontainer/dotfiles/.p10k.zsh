# Generated by Powerlevel10k configuration wizard on 2024-11-17 at 19:06 CST.
...
() {
  ...
  typeset -g POWERLEVEL9K_RVM_BACKGROUND=001
  ...
  function p10k_ruby_version() {
    local version
    version=$(ruby -v 2>/dev/null) # Get the version string, redirect errors
    if [[ -n "$version" ]]; then # Check if the version string is not empty
      # Extract the version number (e.g., 3.2.2 from the string)
      echo "ruby ${version#*ruby }" | awk '{print $2}' | cut -d'p' -f1
    fi
  }

  function prompt_ruby() {
    p10k segment -b 001 -f 0 -i '💎' -t "$(p10k_ruby_version)"
  }

  function instant_prompt_ruby() {
    prompt_ruby
  }
  typeset -g POWERLEVEL9K_RUBY_FOREGROUND=0
  typeset -g POWERLEVEL9K_RUBY_BACKGROUND=001
