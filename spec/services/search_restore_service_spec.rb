...
RSpec.describe SearchRestoreService do
  ...
  describe '.call' do
    ...
    context 'when PG::UndefinedFunction error occurs' do
      ...
      before do
        allow_any_instance_of(SearchRestoreService::AuthenticatedUser).to receive(:call).and_raise(PG::UndefinedFunction)
      end

      it 'rescues error and resets search, returning redirect hash' do
        user.search_params.create!(model: search_key, params: { s: {} })
        expect {
          result = described_class.call(params:, user:, session:, search_key:, action:)
          expect(result).to eq({ action: })
        }.to change { user.search_params.count }.by(-1)
